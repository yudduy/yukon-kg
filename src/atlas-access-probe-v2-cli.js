...
function usage() {
  return [
    "Usage:",
    "  bun run src/atlas-access-probe-v2-cli.js preflight --output <freeze.json>",
    "  bun run src/atlas-access-probe-v2-cli.js run --freeze <freeze.json> --output <results.json>",
    "  bun run src/atlas-access-probe-v2-cli.js analyze --results <results.json> --output <analysis.json>",
  ].join("\n");
}
...
export async function buildAtlasAccessFreeze() {
  ...
  const corePromptHashes = Object.fromEntries(app.cases.map((candidate) => [
    candidate.id,
    sha256(canonicalStringify(buildAccessMessagesV2(candidate))),
  ]));
  ...
  return {
    schema: "yukon.atlas-access-probe-freeze.v2",
    createdAt: null,
    claimScope: "Fresh-case positive duplicate recovery after repairing an output-contract sensitivity failure; mechanism pilot, not full-chain autoresearch efficacy.",
    source: {
      ...
      sourceFiles: [
        ...
        sourceRecord("src/atlas-access-probe-v2.js"),
        sourceRecord("src/atlas-access-probe-v2-cli.js"),
...
function executionOrder(freeze, app) {
  ...
  return calls.sort((left, right) => (
    sha256(`atlas-access-v2:${left.model}:${left.candidate.id}:${left.arm}`)
      .localeCompare(sha256(`atlas-access-v2:${right.model}:${right.candidate.id}:${right.arm}`))
  ));
}
...
async function runProbe(freezePath, outputPath) {
  ...
  const result = existsSync(outputPath) ? readJson(outputPath) : {
    schema: "yukon.atlas-access-probe-results.v2",
    freezeSha256,
    status: "running",
    rows: [],
  ...
  for (let offset = 0; offset < remaining.length; offset += freeze.execution.concurrency) {
    ...
    const outcomes = await Promise.all(batch.map(async ({ model, candidate, arm }) => {
      ...
      for (let attempt = 1; attempt <= freeze.execution.administrativeRetryLimit + 1; attempt += 1) {
        session = await executeAtlasAccessSessionV2({
          candidate,
      ...
      return { administrativeAttempts, failure: null, row: {
        ...
        promptSha256: sha256(canonicalStringify(buildAccessMessagesV2(candidate, {
          pointerRecordId: arm === ATLAS_ACCESS_POINTER_ARM ? app.pointers[candidate.id] : null,
        }))),
        ...session,
      } };
